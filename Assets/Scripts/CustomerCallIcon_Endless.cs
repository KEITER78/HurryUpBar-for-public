...
public class CustomerCallIcon_Endless : MonoBehaviour
{
    ...
    private static Queue<CustomerCallIcon_Endless> waitingCustomers = new Queue<CustomerCallIcon_Endless>(); // 待機中の客を管理するキュー
    ...
    // **エンドレス実装に伴う追加**  
    //public static int currentPhase = 1; // フェーズを管理する静的変数   
    public int maxCustomersInPhase = 4; // 現在のフェーズの最大同時顧客数、フェーズ1では4人   
    public static int activeCustomers = 0; // 現在アクティブな顧客数
    private static List<CustomerCallIcon_Endless> spawnQueue = new List<CustomerCallIcon_Endless>(); // スポーン待機中の客を管理するキュー（フェーズ順に並べる）
    private static bool isSpawning = false; // スポーン処理が動作中かどうかを示すフラグ
    private static SeatManager seatManagerStatic; // SeatManagerの静的参照
    public int phase; // 客のフェーズを管理する変数
    public int spawnOrder = 0; // Inspector で設定するスポーン順序。値が小さいほど先にスポーンされる。

    public TextMeshProUGUI gameOverRevenueText; // GameOverCanvas内の売上表示用テキスト

    private RankingDisplay rankingDisplay; // ランキング表示用のスクリプト
    ...
    void Start()
    {
        ...
        //currentPhase = 1; // フェーズを1にリセット
        //maxCustomersInPhase = 4; // 最大顧客数をフェーズ1のデフォルト値にリセット
        activeCustomers = 0; // アクティブな顧客数を0にリセット
        spawnQueue.Clear(); // スポーン待機中の客リストをクリア
        isSpawning = false; // スポーン処理のフラグをリセット
        seatManagerStatic = null; // SeatManagerの静的参照をリセット

        // フェーズを1に初期化
        phase = 1;

        // フェーズに基づいて変数を設定
        SetVariablesForPhase();
        ...
        // シーン内にある RankingDisplay を１つ見つける
        ...
        // 指定時間後に客をスポーンさせる
        StartCoroutine(DelayWithAction(1f, () => StartCoroutine(SpawnCustomerAfterDelay())));
    ...
    {
        // 個別の客ごとにランダムでドリンクを選択
        selectedDrink = drinkSprites[Random.Range(0, drinkSprites.Length)];
    ...
    {
        // 他のすべての客の選択状態を解除
        CustomerCallIcon_Endless[] allCustomers = FindObjectsOfType<CustomerCallIcon_Endless>();
        foreach (CustomerCallIcon_Endless customer in allCustomers)
        {
        ...
        if (willPay)
        {
            ...
            StartCoroutine(DelayWithAction(0.1f, () =>
            {
                ...
                conversationMessages = new string[]
                {
                    ...
                    $"お客さん「じゃあ{(Random.value > 0.5f ? "カード" : "キャッシュ")}で。」"  // ランダムでカードかキャッシュを選択
                };
    ...
    private bool CalculatePaymentProbability(float totalAmount, float billAmount)
    {
        ...
        {
            return Random.value <= 0.9f * ((100f - satisfactionLevel) / 100f); // 90％×(100 - 不満度)（％）
        }
        ...
        else
        {
            ...
            return Random.value <= probability; // 計算した確率で支払う
        }
    }
    ...
    private void OnBilledAmountSelected()
    {
        ...
        // 確率で支払いが行われるか判定
        if (Random.value <= paymentProbability)
        {
            // 会話の開始（支払い金額の確認）
            StartCoroutine(DelayWithAction(0.1f, () =>
            {
                ...
                $"お客さん「はぁ。じゃあ{(Random.value > 0.5f ? "カード" : "キャッシュ")}で。」"  // ランダムでカードかキャッシュを選択
                };
    ...
    public static void OnSeatAvailable()
    {
        ...
        {
            // 待機している最初の客をスポーンさせる
            CustomerCallIcon_Endless nextCustomer = waitingCustomers.Dequeue();
    ...
    private void MoveToRandomSeat()
    {
        ...
        // ランダムに空き席を選択
        selectedSeatIndex = availableSeats[Random.Range(0, availableSeats.Count)];
    ...
    private void GameOver()
    {
        ...
        // GameOverCanvas内のRevenueTextのアルファを0に設定
        if (gameOverRevenueText != null)
        {
            Color revenueColor = gameOverRevenueText.color;
            revenueColor.a = 0f; // 透明に設定
            gameOverRevenueText.color = revenueColor;
        }

        // GameOverRevenueTextに売上金額を設定
        if (gameOverRevenueText != null)
        {           
            switch (languageIndex)
            {
                case 0: // 英語
                    gameOverRevenueText.text = "Sales: " + Mathf.FloorToInt(dailyRevenue).ToString() + " yen";
                    break;
                case 1: // 日本語
                    gameOverRevenueText.text = "本日の売上: " + Mathf.FloorToInt(dailyRevenue).ToString() + " 円";
                    break;
            }
        }
        ...
        {
            // dailyRevenue を渡してランキング処理を開始
            rankingDisplay.StartRankingProcess(dailyRevenue);
        }
    }
    ...
    private IEnumerator FadeInGameOver()
    {
        ...
        while (elapsed < textFadeDuration)
        {
            ...
            // GameOverRevenueTextもフェードイン
            if (gameOverRevenueText != null)
            {
                Color revenueColor = gameOverRevenueText.color;
                revenueColor.a = alpha;
                gameOverRevenueText.color = revenueColor;
            }
        ...
        // GameOverRevenueTextのアルファを完全に1に設定
        if (gameOverRevenueText != null)
        {
            Color revenueColor = gameOverRevenueText.color;
            revenueColor.a = 1f;
    ...
    private bool IsClosestCustomer()
    {
        ...
        CustomerCallIcon_Endless closestCustomer = null;
    ...
    private void SetVariablesForPhase()
    {
        int N = phase; // フェーズ番号
        ...
        if (N >= 1 && N <= 6)
        {
            maxCustomersInPhase = N + 5; //同時に席に座れる最大数

            float randomValue = Random.value; // 0.0～1.0のランダム値を取得
            if (randomValue < 0.3f)
            {
                idleMaxCount = 2; // 30%
            }
            ...
            {
                idleMaxCount = 3; // 次の40% (0.3～0.7)
            }
            else
            {
                idleMaxCount = 4; // 残り30% (0.7～1.0)
            }

            waitDecayStartTime = 8f * maxCustomersInPhase; //待機中の不満度上昇までの時間
            fastWaitDecayStartTime = waitDecayStartTime + 10f; // 10秒追加
        }
        else if (N >= 7)
        {
            maxCustomersInPhase = 11; //同時に席に座れる最大数
            ...
            waitDecayStartTime = 8f * 10f - 0.5f * (N - 6f); //待機中の不満度上昇までの時間
            fastWaitDecayStartTime = waitDecayStartTime + 10f; // 10秒追加
        }
        else
        {
            Debug.LogError("無効なフェーズ番号: " + N);
        }
        ...
        isSpawned = false;
        isServeMessageDisplayed = false;
        isRotationRestricted = false; // フェーズの要件に応じて設定

        // タイマーをリセット
        callTimeElapsed = 0f;
        waitTimeElapsed = 0f;
        awaitingPaymentTimeElapsed = 0f;

        // 減衰フラグをリセット
        isCallDecaying = false;
        isFastCallDecaying = false;
        isWaitDecaying = false;
        isFastWaitDecaying = false;
        isAwaitingPaymentDecaying = false;
        isFastAwaitingPaymentDecaying = false;
        ...
        // 必要に応じて満足度レベルをリセット
        satisfactionLevel = 0f;
        UpdateSatisfactionBar();
    }

    private void RequestSpawn()
    {
        // スポーン待機キューに phase を優先し、同一 phase では spawnOrder を基準に挿入
        int index = spawnQueue.FindIndex(c =>
            c.phase > this.phase ||
            (c.phase == this.phase && c.spawnOrder > this.spawnOrder)
        );
        if (index >= 0)
        {
            spawnQueue.Insert(index, this);
        }
        else
        {
            spawnQueue.Add(this);
        ...
        if (!isSpawning)
        {
            StartCoroutine(SpawnCustomersFromQueue());
        }
    }

    private static IEnumerator SpawnCustomersFromQueue()
    {
        isSpawning = true;
        ...
        {
            // キューの最初の客を確認
            CustomerCallIcon_Endless customer = spawnQueue[0];

            // アクティブな顧客数が、その客のフェーズの最大顧客数未満になるまで待機
            while (activeCustomers >= customer.maxCustomersInPhase)
            {
                yield return new WaitForSeconds(1f); // 1秒ごとにチェック
            }

            // 席が空いているかチェック
            if (seatManagerStatic.GetAvailableSeats().Count > 0)
            {
                // キューの最初の客を取得して削除
                customer = spawnQueue[0];
